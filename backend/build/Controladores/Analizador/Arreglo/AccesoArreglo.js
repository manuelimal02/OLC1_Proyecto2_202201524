...
class AccesoMatriz extends Instruccion_1.Instruccion {
    constructor(identificador, fila, columna, posicion1) {
        super(new Tipo_1.default(Tipo_1.tipo_dato.VOID), fila, columna);
        this.identificador = identificador;
        this.posicion_1 = posicion1;
    }
    interpretar(arbol, tabla) {
        let valor_variable = tabla.getArreglo(this.identificador);
        if (valor_variable == null) {
            let error = new Errores_1.default("Semántico", "Acceso Arreglo Inválido.", this.fila, this.columna);
            arbol.agregarError(error);
            arbol.setConsola("Semántico: Acceso Arreglo Inválido.\n");
            return error;
        }
        let posicion1 = this.posicion_1.interpretar(arbol, tabla);
        if (posicion1 instanceof Errores_1.default)
            return posicion1;
